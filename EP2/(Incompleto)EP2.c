...
// Ordena por Pontos 
void
ordenaPontos (time * timescampeonato, int notimes)
{
  //Insertion Sort
  int i, j, aux;
  for (i = 0; i < notimes - 1; i++)
    {
      if (timescampeonato[i].PontosGanhos >
	  timescampeonato[i + 1].PontosGanhos)
	{
	  ...
	  while (j >= 0)
		...
		{
		  timescampeonato[j + 1].PontosGanhos =
		    timescampeonato[j].PontosGanhos;
...
void
ordenaSaldoGols (time * timescampeonato, int notimes)
{
  // Insertion Sort
  int i, j, aux;
  for (i = 0; i < notimes - 1; i++)
      ...
      if (timescampeonato[i].SaldoDeGols > timescampeonato[i + 1].SaldoDeGols)
	{
	  ...
	  while (j >= 0)
		...
		{
		  timescampeonato[j + 1].SaldoDeGols =
		    timescampeonato[j].SaldoDeGols;
...
int
encontratime (time * timescampeonato, char *nome, int notimes)
{
...
int
crialistatimes (time * timescampeonato, jogo * dadosjogos, int numerojogos)
{
...
// Computa dados times
void
computadadostimes (time * timescampeonato, int notimes, jogo * dadosjogos,
		   int numerojogos)
{
...
// GolsSofridos, GolsMarcados, Golsaverage, SaldoDeGols, PontosGanhos.
  ...
  timescampeonato->SaldoDeGols =
    timescampeonato->GolsMarcados - timescampeonato->GolsSofridos;
  if (timescampeonato->GolsSofridos != 0)
    {
      timescampeonato->GolAverage =
	timescampeonato->GolsMarcados / timescampeonato->GolsSofridos;
    }
  else
    {
  ...
  timescampeonato->PontosGanhos =
    (timescampeonato->Vitorias * 3) + (timescampeonato->Empates);
      ...
      for (int j = 0; j < numerojogos; j++)
	{
	  if (timescampeonato[i].nome == dadosjogos[j].local)
	  ...
	  else if (timescampeonato[i].nome == dadosjogos[j].visitante)
	    {
	      ...
	      timescampeonato[i].GolsMarcados += dadosjogos[j].golslocal;
	      if (dadosjogos[j].golsvisitante > dadosjogos[j].golslocal)
		timescampeonato[i].Vitorias++;
	      else if (dadosjogos[j].golsvisitante == dadosjogos[j].golslocal)
		timescampeonato[i].Empates++;
	      else
		timescampeonato[i].Derrotas++;
	    }
...
void
imprimeclassificacao (time * timescampeonato, int notimes)
{
...
main ()
{
  ...
  printf
    ("Entre os jogos no formato \"time local, time visitante, golslocal, golsvisitante\" (gols local negativo encerra a entrada de dados)\n");
  ...
  for (i = 0; i < nojogos; ++i)
    printf ("%d:%s,%s,%d,%d\n", i + 1, jogos[i].local, jogos[i].visitante,
	    jogos[i].golslocal, jogos[i].golsvisitante);
