...
datasMutuas (int d, int m, int a)
{
  ...
  if (a % 4 == 0 && (a % 100 != 0 || a % 400 == 0))
    ...
    {
      diasRestantesDoAno = 365 - (d + ((m - 1) * 30));
    }
...
int
verificaDataValida (int d, int m, int a)
{
  if (a > 0)
    {
      if (m == 1 || m == 3 || m == 5 || m == 7 || m == 8 || m == 10
	...
	{			//meses com 31 dias
	  if (d >= 1 && d <= 31)
	    {
	      return 1;
	    }			//retorna true se o dia é válido
	  else
	    {
	      return 0;
	    }			//retorna false se o dia é inválido
	}
      else if (m == 4 || m == 6 || m == 9 || m == 11)
	{			//meses com 30 dias
	  if (d >= 1 && d <= 30)
	...
	{
	  return 0;
	}			//retorna false se mês invalido
    }
  else
    return 0;			//retorna false se ano inválido
}
